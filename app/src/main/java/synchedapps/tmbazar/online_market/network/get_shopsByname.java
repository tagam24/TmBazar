...
import synchedapps.tmbazar.online_market.post_tab.fragment_postmarkets;
    ...
    public static void  get_Data(){
        ...
        get_data=new Thread(new Runnable() {
            ...
            public void run() {
                try{
                    URL url;
                     url=new URL("http://"+ Api.url+"search_shop.php");
                    Log.d("url",""+url);
                    ...
                    String post_data =URLEncoder.encode("Region", "UTF-8") + "=" + URLEncoder.encode(Constants.Location, "UTF-8")+"&"+
                            URLEncoder.encode("bazar", "UTF-8") + "=" + URLEncoder.encode(Constants.bazar, "UTF-8")+"&"+
                            URLEncoder.encode("category", "UTF-8") + "=" + URLEncoder.encode(Constants.market_category, "UTF-8")
                            +"&"+
                            URLEncoder.encode("type", "UTF-8") + "=" + URLEncoder.encode(Constants.hb, "UTF-8")
                            +"&"+
                            URLEncoder.encode("shop", "UTF-8") + "=" + URLEncoder.encode(Constants.shcname, "UTF-8")+"&"+
                ...
                try{//bashda data basadan hemme informasiyasyny alyas
                 ...
                 shops m;
                    for(int i=0; i<jsonArray.length();i++){
                        ...
                        m=new shops(data.optString("id"),data.optString("category"),data.optString("name"),
                                data.optString("nomer"),data.optString("content"),data.optString("skidka"),data.optString("image"));
                        if(!Constants.shopid.contains(data.optString("id"))){ Constants.shopses.add(m); Constants.shopid.add(data.optString("id"));}
                    }
if(!Constants.market_category.equals(""))  post_markets.s1.sendEmptyMessage(1);
                     else
        if (!Constants.hb.equals("Bazarlar")&&
        !Constants.hb.equals("Söwda Merkezler"))
    post_markets.s1.sendEmptyMessage(1); else
